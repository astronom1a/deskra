  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Database Pejabat</h1>
          <p className="text-gray-500 text-sm mt-1">Kelola data nama, NPK & jabatan pejabat aktif</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white border border-primary-200 rounded-xl p-5 mb-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <p className="font-semibold text-gray-700">{editId ? 'Edit Pejabat' : 'Tambah Pejabat Baru'}</p>
            <button onClick={() => setShowForm(false)}><X size={16} className="text-gray-400 hover:text-gray-600" /></button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-medium text-gray-500 mb-1 block">NPK / No. Pegawai</label>
              <input
                value={form.npk}
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <label htmlFor="aktif" className="text-sm text-gray-600">Aktif</label>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 text-sm">Belum ada data pejabat.</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {data.map((row, i) => (
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{row.npk || '—'}</td>
                  <td className="px-5 py-3.5 font-medium text-gray-800">{row.nama}</td>
                  <td className="px-5 py-3.5 text-gray-600">{row.jabatan}</td>
